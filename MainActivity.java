...
import com.example.robottest3.databinding.ActivityMainBinding;
...
public class MainActivity extends RobotActivity implements RobotLifecycleCallbacks {
    ...
    public void onRobotFocusGained(QiContext qiContext) {
        ...
        RecyclerViewAdapter mainAdapter = new RecyclerViewAdapter(new ArrayList<>());
        RecyclerView.LayoutManager layoutManager = new LinearLayoutManager(this);
        RecyclerView chatRecyclerView = binding.chatRv;
        chatRecyclerView.setLayoutManager(layoutManager);
        chatRecyclerView.setAdapter(mainAdapter);

        // Create a new QiChatbot.
        PepperChatbot My_Chatbot = new PepperChatbot(qiContext);
        My_Chatbot.setBotAdapter(mainAdapter);
...
class PepperChatbot extends BaseChatbot {
    ...
    RecyclerViewAdapter botAdapter;

    public void setBotAdapter(RecyclerViewAdapter mainAdapter){
        this.botAdapter = mainAdapter;
    }

    public void addMessage(ChatElement message){
        if(botAdapter != null){
            botAdapter.addMessage(message);
        }
    }
    ...
    public StandardReplyReaction replyTo(Phrase phrase, Locale locale) {
        ...
        if (phrase.getText() != "") {
            botAdapter.addMessage(new ChatElement("user", phrase.getText()));
        }

        String Ans = gpt.answerQuestion(phrase.getText());
        Log.e("text", Ans);
        if (Ans != "") {
            botAdapter.addMessage(new ChatElement("me", Ans));
            ...
            return new StandardReplyReaction(
                    new MyChatbotReaction(getQiContext(), "Sorry, I didn't get that!"),
    ...
    public String answerQuestion(String question) {
        ...
        try {
            ...
            if (responseCode == HttpURLConnection.HTTP_OK) {
                ...
                String answer = removeAfterBlock(textValue);
                return answer;
            }
            else {
